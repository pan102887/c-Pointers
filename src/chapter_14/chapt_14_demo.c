...
#define CP_CH14_PRINT(FORMAT, ...) printf("the value of " #__VA_ARGS__ " is: " #FORMAT "\n", __VA_ARGS__)
static void print_test(void)
{
    CP_CH14_PRINT("%d, %d, %d", 10, 20, 3 + 8);
}

#define CP_CH14_ADD_TO_SUM(sum_number, value) sum##sum_number += value
static void add_to_sum_test(void)
{
    int sum1 = 0;
    int sum2 = 0;
    int sum_number = 1;
    CP_CH14_ADD_TO_SUM(1, 10);
    CP_CH14_ADD_TO_SUM(2, 20);
    CP_CH14_ADD_TO_SUM(_number, 30);
