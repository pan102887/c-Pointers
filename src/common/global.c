...
#define CP_DIVIDING_LINE_LEN 100
...
void print_dividing_line(const char *str)
{
    ...
    if (str_len >= CP_DIVIDING_LINE_LEN)
    {
        printf("%s\n", str);
        return;
    }

    size_t prefix_len = (CP_DIVIDING_LINE_LEN - str_len) / 2;
    size_t subfix_len = CP_DIVIDING_LINE_LEN - str_len - prefix_len;
...
void multi_dividing_line(int n_str, char const *str, ...)
{
    ...
    str_len_sum += n_str - 1;
    size_t prefix_len = (CP_DIVIDING_LINE_LEN - str_len_sum) / 2;
    size_t subfix_len = CP_DIVIDING_LINE_LEN - str_len_sum - prefix_len;
    print_prefix(prefix_len, '-');
    for (int i = 0; i < n_str; i++)
    {
