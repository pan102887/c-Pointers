...
static inline node_wrapper_ptr node_create()
{
    node_wrapper_ptr node = CP_NEW(node_wrapper, 1);
    node->parent = NULL;
    node->type = UNKNOWN;
    node->node = NULL;
    return node;
...
static inline two_node_ptr two_node_create()
{
    two_node_ptr node = CP_NEW(two_node, 1);
    node->key = NULL;
    node->value = NULL;
    node->left = NULL;
    node->right = NULL;
...
static inline three_node_ptr three_node_create()
{
    three_node_ptr node = CP_NEW(three_node, 1);
    node->left_key = NULL;
...
extern tt_tree_entry_ptr tt_tree_create(tt_compare compare, key_cpy kconvert, value_cpy vconvert, tt_destroy destroy)
{
    ...
    tt_tree_entry_ptr entry = CP_NEW(tt_tree_entry, 1);
    entry->root = NULL;
...
static inline tt_internal_key_type key_copy(tt_key_type key)
{
    ...
    return strcpy(CP_NEW(char, strlen(key) + 1), key);
}
...
static inline tt_internal_value_type value_copy(tt_value_type value)
{
    ...
    return strcpy(CP_NEW(char, strlen(value) + 1), value);
}
