...
#define CP_ARRAY_LIST_DEFAULT_CAPACITY 10
#define CP_ARRAY_LIST_MAX_ARRAY_SIZE ((size_t)pow(2, 32) - 1)
...
extern m_array_list *m_array_list_new()
{
    ...
    list->capacity = CP_ARRAY_LIST_DEFAULT_CAPACITY;
    list->size = 0;
    list->element_data = malloc(sizeof(void *) * CP_ARRAY_LIST_DEFAULT_CAPACITY);
    return list;
}
...
static inline void grow(m_array_list *list, size_t min_capacity)
{
    ...
    if (min_capacity > CP_ARRAY_LIST_MAX_ARRAY_SIZE)
    {
        perror("min_capacity must be less than CP_ARRAY_LIST_MAX_ARRAY_SIZE");
        exit(EXIT_FAILURE);
    }
    ...
    if (new_capacity > CP_ARRAY_LIST_MAX_ARRAY_SIZE)
    {
        new_capacity = CP_ARRAY_LIST_MAX_ARRAY_SIZE;
    }
